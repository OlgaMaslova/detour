...
- **A place's total standing is global, and shown.** On the place page, the signal badge merges the global recommender count with the global Been & loved count into one distinct-member total; the states cannot overlap for one member. Its label and tooltip add how much of that comes from members the caller can see: "Recommended by 10 Detourists, 4 in your circle". Cards and map previews, which do not carry corroboration data, continue to show the recommendation count only. This is presentation only: `signal_count`, publication, visibility, and ordering remain recommendation-only. The global figures are emitted **only** for a place the caller can already see, so they can never disclose that a place exists; notes, hidden names, dates, and photos remain invisible. A member may infer that members beyond their circle stand behind the place, but learns nothing about who or what was said.
