# Detour community waiting-list and trust policy

**Status:** governing policy for the active community place workflow.
**Applies to:** member verification, private endorsements, community waiting-list entries, recommendations, shares, historical visit evidence/submissions, and public community selections.
...
1. A verified member proposes a place with a display venue name, city, country, and a meaningful personal recommendation note.
...
3. Each verified member may add at most one `community_recommendations` record to an entry. A qualifying record is one independent signal; repeated attempts by the same member are rejected and do not increase the count.
4. At the third independent verified-member signal, the backend automatically publishes the place. No curator or editorial approval is required for this community-loop publication.
...
- A verified sender may share an existing waiting-list entry only when the sender already participates in that entry.
...
- If the recipient is or later becomes a verified member and submits their own meaningful recommendation against that entry, that recommendation is their independent signal.
- Self-targeted shares, nonexistent recipients, unverified senders, published-entry shares, and invalid or missing place/note data are rejected.
...
- `endorsements`, `visit_evidence`, and `detour_submissions` remain private under their existing rules.
...
## 6. Member verification and endorsements

Email verification protects account control but does not by itself grant Detour community trust.

- Existing members who were validly verified under the founding process retain the hidden `founding_verified` trust fact unless an authorized curator deliberately revokes it through the existing founding-verification path.
- After the founding cohort, a non-founding member becomes verified only when they hold at least two independent active endorsements from distinct members who are currently verified.
- Each verified member may hold at most three active outgoing endorsements.
- Self-endorsement and duplicate endorser/endorsee pairs are prohibited.
- An endorser may revoke their own endorsement. The backend recalculates the affected member and propagates a status change through downstream endorsements so an unverified member is not retained as a trust source.

Endorsements are private trust records. They are not place signals, public recommendations, named attribution, or permission to expose the trust graph. The existing endorsement behavior and founding-verification route remain unchanged.
...
- visit-evidence counts do not grant member verification or add place signals;
...
- only verified members can create recommendations or send shares;
- one verified member can contribute only one signal per entry;
...
- a shared recipient's own later recommendation counts independently;
...
- founding verification, endorsement propagation, the public catalogue, and historical `detour_submissions`/visit-evidence behavior continue to work unchanged.
