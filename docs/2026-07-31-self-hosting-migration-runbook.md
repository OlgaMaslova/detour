...
- `[DONE]` **No platform payments collections to clean up — checked 2026-07-31.** `1768100000_baseline_schema.js` and the old `AGENTS.md` both described five platform-managed collections (`supernaut_payments_settings`, `supernaut_products`, `supernaut_customers`, `supernaut_subscriptions`, `supernaut_payments`), which reads as though the platform had provisioned them into this database. It never did. All five return 404 on `https://api.takedetour.app` (against `venues` → 200, so the API was reachable), and none is in the local `_collections` table either. Production holds exactly the 14 collections the migrations declare. The baseline's "deliberately absent" meant absent, full stop.
- `[TODO]` The outbound `User-Agent` in `pb_hooks/community_waitlist.js` (3 places) advertises `olga@supernaut.dev` as the contact for OpenStreetMap/Nominatim requests, on a mailbox that is being shut down. Nominatim's usage policy expects a reachable address, so replace it with a `takedetour.app` one rather than dropping the field.
- `[TODO]` Two public-recommendation queries in `pb_hooks/main.pb.js` (lines ~508 and ~665) hard-code `AND LOWER(TRIM(m.email)) != 'agent@detour.supernaut.to'` to keep an agent account out of public surfaces. **Do not simply delete these** — if that member still exists, dropping the filter publishes its recommendations. Both queries already exclude `internal_member = TRUE`, so the clean fix is to set `internal_member` on that account (or delete it), confirm it no longer appears in `/api/detour/public-recommendations`, and only then remove the email literal.

**Naming cleanup already done (2026-07-31):** `Dockerfile.supernaut-pocketbase` → `Dockerfile` (with `fly.toml` and `.github/workflows/deploy.yml` updated), the readiness route `GET /api/supernaut/ready` → `GET /api/detour/ready` (it had no callers — Fly's health check uses `/api/health`), and the "provisioned by Supernaut" line dropped from `pb_public/index.html`. `README.md` and `AGENTS.md` no longer mention the old platform at all; the remaining references are the `[TODO]`s above and this runbook, which has to name what it migrated off.
